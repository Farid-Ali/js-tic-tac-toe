...
/**
 * This factory function hold the player information
 * @param {*} sign the player's choosen sign
 */
...
/**
 * Copy this code from internet, should be examined later
 */
...
/**
 * This module controll the hole game
 */
...
const gameController = (() => {
   ...
   * If someone filled a column, it returns ture, else it returns false.
   * @param {gameBoard} board - call the gameBoard
   */
  ...
  const _checkForDiagonals = (board) => {
    let diagonalOne = [board.getField(0), board.getField(4), board.getField(8)];
    let diagonalTwo = [board.getField(6), board.getField(4), board.getField(2)];

    if (
      diagonalOne.every((field) => field == 'X') ||
      diagonalOne.every((field) => field == 'O')
    ) {
      return true;
    } else if (
      diagonalTwo.every((field) => field == 'X') ||
      diagonalTwo.every((field) => field == 'O')
    ) {
      return true;
    } else {
      return false;
  ...
  /**
   * Changes the sign of the human player to 'sign' and the AI player to the other sign.
   ...
   * If the game finished, it disables the buttons.
  ...
  const playerStep = (num) => {
    ...
    if (field == undefined) {
      ...
      if (checkForWin(gameBoard)) {
        ...
        //FIXME should sleep for some time before calling bellow function
        aiStep();
        if (!checkForWin(gameBoard)) {
          displayController.active();
        }
      }
    } else {
